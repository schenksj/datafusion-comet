...
pub(crate) fn init_datasource_exec(
    ...
    // Honour Spark's `ignoreMissingFiles` by wrapping the final FileSource so
    // its FileOpener swallows object-store NotFound errors as empty streams.
    // Wrapped after pushdown to preserve all optimizer behaviour.
