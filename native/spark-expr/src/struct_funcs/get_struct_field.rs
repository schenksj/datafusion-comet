...
impl GetStructField {
    ...
    /// the child is null). Surfaced by Delta checkpoint reads that filter
...
mod tests {
    ...
    // A field of a NULL struct must be NULL even when the child buffer holds a non-null value
    // at that row (Arrow child validity is independent of the parent struct's null mask). This
    // guards the fix for the Delta-checkpoint `checkpointMetadata.version IS NOT NULL` leak.
    #[test]
    fn field_of_null_struct_is_null() {
        // Child is non-null at every row; the struct is null at rows 1 and 3.
