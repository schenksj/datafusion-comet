...
pub trait SparkUnsafeObject {
    ...
    ///
    /// Spark's `UnsafeRow.getUTF8String` wraps the bytes via `UTF8String.fromAddress`
    /// with no UTF-8 validation, and Spark's `cast(BinaryType -> StringType)` is a
    /// zero-copy reinterpret that can leave arbitrary bytes in a `StringType` column
    /// (e.g. Delta's Z-Order `interleave_bits(...).cast(StringType)`). Strict
    /// `from_utf8(...).unwrap()` here panics on those rows even though Spark itself
    /// treats them as opaque. We use `from_utf8_lossy`: returns the original `&str`
    /// borrow for valid UTF-8 (zero-cost) and a `String` with `U+FFFD` replacements
    /// for invalid bytes (defined behaviour, no UB). The replacement-char round-trip
    /// only fires on the binary-cast-to-string case where the bytes weren't intended
    /// for display anyway. Avoids `from_utf8_unchecked`, which would construct a
    /// `&str` from arbitrary bytes -- the Rust reference defines that as undefined
    /// behaviour even though the bytes only get copied downstream, and downstream
    /// Arrow ops that internally use `str::from_utf8_unchecked` on the StringArray
    /// buffer would propagate the UB.
