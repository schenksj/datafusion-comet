...
pub trait SparkUnsafeObject {
    ...
    /// (e.g. Delta's Z-Order `interleave_bits(...).cast(StringType)`). Strict validation
    /// here panics on those rows even though Spark itself treats them as opaque. The
    /// bytes flow directly into Arrow's `StringBuilder::append_value`, which copies them
    ...
    fn get_string(&self, index: usize) -> &str {
        ...
        // SAFETY: matches Spark's `UTF8String.fromAddress` no-validate semantics. See doc above.
