...
//! Spark-parity UTF-8 decoding for `StringType` byte buffers.
...
/// `U+FFFD`. Matching the JDK byte-for-byte means a Comet shuffle / cast of arbitrary bytes renders
/// identically to Spark. The per-class malformed lengths below (E0/ED overlong & surrogate handling,
/// F0/F4 range checks) match the observable replacement behavior of the JDK UTF-8 decoder; they were
/// determined from observed `new String(bytes, UTF_8)` output, not by reviewing the OpenJDK source.
