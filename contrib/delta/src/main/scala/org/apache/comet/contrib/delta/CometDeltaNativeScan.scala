...
// contrib/delta/native/src/proto/delta_operator.proto. Package matches the
// `package comet.contrib.delta;` declaration in that .proto file.
import comet.contrib.delta.DeltaOperator.{DeltaScan, DeltaScanCommon, DeltaScanTaskList}
...
object CometDeltaNativeScan extends CometOperatorSerde[CometScanExec] with Logging {
  ...
  override def convert(
      ...
      if (!taskList0.getColumnMappingsList.isEmpty || tableColumnMappingMode.isEmpty) {
        ...
        if (logicalToPhysical.isEmpty) {
          ...
          logicalToPhysical.foreach { case (logical, physical) =>
            b.addColumnMappings(
              DeltaOperator.DeltaColumnMapping
                .newBuilder()
    ...
    // Wrap the DeltaScan in the generic ContribOp envelope so core's planner
    // dispatches via lookup_contrib_planner_by_kind("delta-scan") rather than
    // having a hard-coded OpStruct::DeltaScan arm (that arm doesn't exist in
    // post-SPI core). The Rust side decodes payload back into DeltaScan via
    // its own prost-generated types in contrib/delta/native/src/proto/.
    val envelope = ContribOp
      .newBuilder()
      .setKind(CometDeltaNativeScan.DeltaScanKind)
      .setPayload(deltaScanBuilder.build().toByteString)
      .build()
  ...
  private def splitTasks(
    ...
    tasks.flatMap { task =>
      ...
      if (size <= msb) Seq(task)
        ...
        val chunks = scala.collection.mutable.ArrayBuffer[DeltaOperator.DeltaScanTask]()
        var offset = 0L
        while (offset < size) {
  ...
  private def prunePartitions(
      tasks: Seq[DeltaOperator.DeltaScanTask],
      scan: CometScanExec,
      partitionSchema: StructType): Seq[DeltaOperator.DeltaScanTask] = {
  ...
  private def buildTaskListFromAddFiles(
      ...
      : DeltaOperator.DeltaScanTaskList = {
    val tlBuilder = DeltaOperator.DeltaScanTaskList.newBuilder()
    ...
    addFiles.foreach { af =>
      ...
      val taskBuilder = DeltaOperator.DeltaScanTask.newBuilder()
      ...
      af.partitionValues.foreach { case (k, v) =>
        ...
        val pvBuilder =
          DeltaOperator.DeltaPartitionValue.newBuilder().setName(logicalName)
