...
case class CometDeltaCdfScanExec(
  ...
  // Memoised (lazy val, like the sibling CometNativeScanExec/CometIcebergNativeScanExec) rather than
  // a `def`: a CDF scan's `subRanges` are fixed at planning (no DPP pruning to re-resolve at
  // execution, unlike CometDeltaNativeScanExec.perPartitionData), so the protobuf bytes are built
  // exactly once and reused across the fused-block (findAllPlanData) and standalone (doExecuteColumnar)
  // reads. @transient so the computed bytes aren't serialised with the exec (they ship via CometExecRDD).
  @transient override lazy val commonData: Array[Byte] = nativeOp.getDeltaScan.getCommon.toByteArray
  ...
  // spliced range.
  @transient override lazy val perPartitionData: Array[Array[Byte]] = subRanges.map { case (start, end) =>
