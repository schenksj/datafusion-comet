...
- JAR without contrib, dylib with → contrib classes and the `CometScanContrib` service file are simply absent; core's registry stays empty; all Delta queries go through Spark's reader
...
The contrib plugs into core through the JDK `ServiceLoader`, via two service
files packaged only under `-Pcontrib-delta`:
`META-INF/services/org.apache.comet.rules.CometScanContrib` (scan claim/decline,
incl. CDF) and `META-INF/services/org.apache.spark.sql.comet.PlanDataInjector`
(per-partition plan data). It does NOT register a
`SparkSessionExtensionsProvider` — it rides on Comet's existing extension.
...
registry is empty, every hook returns `None`, and Spark's unaccelerated path
runs.

## What `libcomet` looks like

The dylib produced by `cargo build --release -p datafusion-comet --features contrib-delta`
contains:
...
Default build (no feature) omits the contrib code entirely. The generic
`OpStruct::ContribScan` dispatcher arm in
`native/core/src/execution/planner.rs` exists unconditionally (the proto variant
exists regardless of Cargo features, and the `match` must stay exhaustive);
under a default build no contrib handler is compiled in, so it returns a clear
error naming the unhandled `type_url` if a `contrib_scan` proto somehow
arrives:
