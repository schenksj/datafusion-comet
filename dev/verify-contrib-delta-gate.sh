...
#
# Grant the exception through the environment rather than `git config --global`, so it lasts
# exactly as long as this process and its children. Writing it to the global config would outlive
# the script -- including when a check below exits non-zero -- leaving the setting in the
# developer's ~/.gitconfig; `--add` also appended a duplicate entry on every run. The previous
# `safe.directory=*` was worse still: it disabled git's ownership safeguard for every repository
# on the machine, not just this checkout.
#
# GIT_CONFIG_COUNT/KEY/VALUE needs git >= 2.31; CI images are well past that. Anything older
# simply does not get the exception, which is the pre-existing behaviour for a normally-owned
# checkout anyway.
# APPEND at the current count rather than assuming zero. GIT_CONFIG_COUNT/KEY_n/VALUE_n is a list
# the caller may already be using -- CI images and wrapper scripts pass proxies, credential helpers,
# URL rewrites and `protocol.*.allow` this way. Writing KEY_0 with COUNT=1 would silently drop all
# of it for every git this script (and Cargo, which shells out to git for the Iceberg git
# dependency) runs.
