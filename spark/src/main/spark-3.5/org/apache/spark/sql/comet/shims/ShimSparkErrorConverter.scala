...
object ShimSparkErrorConverter {
  ...
  /**
   * Wrap a native parquet failure in the FAILED_READ_FILE-shaped SparkException Spark itself
   * produces when its own parquet reader fails. Mirrors the spark-4.x shim of the same name.
   * `QueryExecutionErrors.cannotReadFilesError(Throwable, String)` has the same signature in
   * Spark 3.5, so the implementation is identical.
   */
