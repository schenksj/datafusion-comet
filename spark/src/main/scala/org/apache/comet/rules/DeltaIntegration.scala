...
object DeltaIntegration extends org.apache.spark.internal.Logging {
  ...
  private def scanRuleCls: Option[Class[AnyRef]] =
    scanRuleLookup.getOrElse {
      // ClassLoaders.loadClass resolves via Spark's thread-context ClassLoader first (where
      // user/vendor jars are wired), matching IcebergReflection's convention, then caches the
      // result so the lookup is a single reflection cost per JVM.
      val cls =
        try {
          Some(ClassLoaders.loadClass(ScanRuleClass).asInstanceOf[Class[AnyRef]])
  ...
  private def serdeCls: Option[Class[AnyRef]] =
    serdeLookup.getOrElse {
      val cls =
        try {
          Some(ClassLoaders.loadClass(SerdeClass).asInstanceOf[Class[AnyRef]])
        } catch { case _: ClassNotFoundException => None }
  ...
  // Cached Delta scan handler, resolved once per JVM (mirrors the binding caches above): the
  // contrib's serde companion is looked up reflectively and the (`None` vs handler) result is
  // memoised so CometExecRule pays no reflective field round-trip per Delta marker node.
  @volatile private var scanHandlerCache: Option[Option[CometOperatorSerde[_]]] = None
   ...
   */
  def scanHandler: Option[CometOperatorSerde[_]] =
    scanHandlerCache.getOrElse {
      // Annotate the result type explicitly: Scala 2.12 (Spark 3.4 builds) cannot infer the
      // existential `CometOperatorSerde[_]` as `flatMap`'s type parameter without an expected
      // type here (2.13 can). The original un-cached `def` got it from the method return type.
      val handler: Option[CometOperatorSerde[_]] = serdeCls.flatMap { cls =>
        try {
          val module = cls.getField("MODULE$").get(null)
