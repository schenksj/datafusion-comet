...
private[comet] object PlanDataInjector {
  ...
  def injectPlanData(
    ...
    // O(1) by op kind, then a canInject confirm (which may inspect detail fields like
    // `hasCommon` / `!hasFilePartition`). Most operators in any tree are non-scan and skip.
